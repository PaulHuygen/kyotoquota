...
\makeindex
\newcommand{\thedoctitle}{Dynamically set disk quota}
\newcommand{\theauthor}{Paul Huygen <paul.huygen@@huygen.nl>}
\newcommand{\thesubject}{Dynamically set disk quota}
\title{\thedoctitle}
\author{\theauthor}
\date{\today \\ 10:30~h.}
% Packages.
...
@d quota settings @{max_quota_perc=20
...
@d quota settings @{minfreespace_perc=5
maxfreespace_perc=40
...
@d quota settings @{reduction_perc=95
expansion_perc=105
...
group-id 100) and a group ``studs'' with group-id
1035. The group-id is the fourth item in the ``passwd
file'' (\verb|/etc/passwd|).

@d variables @{usergroup_id=100
studgroup_id=1035
...
@d find out free diskspace @{disk_capacity=`df /dev/vda3 2>/dev/null | gawk 'NR==2 {print $2}'`
...
\verb|a_kyotoquota.w|. Figure~\ref{fig:fileschema}
\begin{figure}[hbtp]
  \centering
...
To make usable things from the raw input \verb|a_kyotoquota.w|, do the following:
...
@d expliciete make regels @{m4_kyotoquota.w : a_kyotoquota.w
...
@d  expliciete make regels @{kyotoquota.w : m4_kyotoquota.w
...
\verb|kyotoquota.bib|. To create this file, copy the auxiliary file
to another file \verb|auxfil.aux|, but replace the argument of the
command \verb|\bibdata{kyotoquota}| to the names of the bibliography
...
@d expliciete make regels @{bibfile : kyotoquota.aux /home/paul/bin/mkportbib
...
@d make targets @{pdf : kyotoquota.pdf
...
@d directories to create @{../nuweb/bin @| @}

@d expliciete make regels  @{$(W2PDF) : kyotoquota.w
	$(NUWEB) kyotoquota.w
@| @}
...
# 20161019 at 1030h: Generated by nuweb from a_kyotoquota.w
...
@d expliciete make regels @{m4_htmlsource : kyotoquota.w
...
@d expliciete make regels  @{m4_htmlbibfil : m4_anuwebdir/kyotoquota.bib
...
@d run tex4ht @{tex '\def\filename{{kyotoquota}{idx}{4dx}{ind}} \input idxmake.4ht'
...
@d make targets @{sources : kyotoquota.w $(DIRS)
	$(NUWEB) kyotoquota.w
